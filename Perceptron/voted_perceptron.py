...
def voted_perc_pred(test_data, wm,c):
...
# run voted perceptron and calculate error
def voted_perceptron(train_data, test_data, w, b, rate, T):
    [wm,c] = voted_perc_train(train_data, w, b, rate, T)    
    err = voted_perc_pred(test_data, wm,c)
    return err, each_output(wm,c)
#----------------------------------Report------------------------------------
rate = 1    
T =10
...
for t in range(1,T+1):
    print('T= ', t)
    print('error',voted_perceptron(training_list, testing_list, np.zeros(Num_attr), 0, rate, t)[0])
    print('weight and counts',np.round(voted_perceptron(training_list, testing_list, np.zeros(Num_attr), 0, rate, t)[1],2))
